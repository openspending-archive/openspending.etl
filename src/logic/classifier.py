from openspending.etl.ui.lib.util import check_rest_suffix
from openspending.etl.ui.model import Classifier


def create_classifier(name, taxonomy, label=u'', description=u'',
                      **classifier):
    '''create a :class:openspending.etl.ui.model.`classifier`. The ``name`` has to
    ...
    Returns: An :class:`openspending.etl.ui.model.Classifier` object
...
def get_classifier(name, taxonomy):
    ...
    Returns: An :class:`openspending.etl.ui.model.Classifier` object if found or
    ``None``.
    '''
    query = {'name': name, 'taxonomy': taxonomy}
    return Classifier.find_one(query)
